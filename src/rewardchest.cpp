...
ReturnValue RewardChest::queryAdd(int32_t, const Thing& thing, uint32_t, uint32_t flags, Creature*) const
{
...
void RewardChest::postAddNotification(Thing* thing, const Cylinder* oldParent, int32_t index, cylinderlink_t)
{
	...
	if (Container* container = thing->getContainer()) {
		if (RewardContainer* rewardContainer = container->getRewardContainer()) {
